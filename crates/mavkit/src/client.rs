...
fn regex_extract(re: &str, output: &str) -> Result<String> {
    ...
    let capture = regex
        .captures(output)
        .ok_or_else(|| anyhow!("Unexpected output from mavkit-client"))?
        .get(1)
        .ok_or_else(|| anyhow!("Unexpected output from mavkit-client"))?
        .as_str()
        .to_string();
...
impl MavkitClient {
    ...
    fn command(&self) -> Command {
        ...
        if self.disable_disclaimer {
            command.env("MAVRYK_CLIENT_UNSAFE_DISABLE_DISCLAIMER", "Y");
        }
