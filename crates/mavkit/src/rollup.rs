...
pub struct MavkitRollupNode {
    /// Path to the mavkit-smart-rollup-node binary
    pub mavkit_rollup_node_bin: Option<PathBuf>,
    /// Path to the mavkit-smart-rollup-node directory
    pub mavkit_rollup_node_dir: PathBuf,
    /// Path to the mavkit-client directory
    pub mavkit_client_dir: PathBuf,
    /// RPC endpoint for the mavkit-node
    pub endpoint: String,
}

impl MavkitRollupNode {
    fn command(&self) -> Command {
        let mut command = Command::new(path_or_default(
            self.mavkit_rollup_node_bin.as_ref(),
            "mavkit-smart-rollup-node",
        ));
        command.args([
            "--base-dir",
            self.mavkit_client_dir.to_str().expect("Invalid path"),
            "--endpoint",
            &self.endpoint,
        ]);
    ...
    pub fn run(
        ...
        Ok(self
            ...
            .args([
                ...
                operator,
                "--data-dir",
                self.mavkit_rollup_node_dir.to_str().expect("Invalid path"),
                "--rpc-addr",
                addr,
                "--rpc-port",
                &port.to_string(),
