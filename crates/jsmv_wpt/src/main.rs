    ...
    match command {
        Command::Doctor => {
            ...
            if !diagnosis.hosts_configured {
                println!("WPT hosts are not configured. Please run `jsmv-wpt init` and try again.");
                println!("Alternatively, you can configure them manually by appending `jsmv_wpt/hosts` to your `/etc/hosts` file:");
                println!("      cat jsmv_wpt/hosts | sudo tee -a /etc/hosts");
            }
