...
use jsmv_core::{host_defined, Api, Runtime};
use jsmv_wpt::{
...
/// The test harness report jsmv API (bound to the global object)
...
impl TestHarnessReportApi {
    /// The add_result_callback function for jsmv's test harness reports
    ///
    /// More information:
    ///  - [wpt documentation][wpt]
    ///
...
impl jsmv_core::Api for TestHarnessReportApi {
...
pub fn register_apis(context: &mut Context<'_>) {
    ...
    jsmv_api::http::header::HeadersApi.init(context);
    jsmv_api::encoding::EncodingApi.init(context);
}
