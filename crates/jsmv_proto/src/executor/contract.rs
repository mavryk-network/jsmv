...
use jsmv_api::http::{body::HttpBody, request::RequestClass, response::Response};
use jsmv_api::{http::request::Request, js_log::set_js_logger};
use jsmv_core::native::JsNativeObject;
use jsmv_core::{
    host::HostRuntime,
    host_defined,
    kv::{Kv, Transaction},
...
fn register_web_apis(realm: &Realm, context: &mut Context<'_>) {
    realm.register_api(jsmv_api::url::UrlApi, context);
    realm.register_api(jsmv_api::urlpattern::UrlPatternApi, context);
    realm.register_api(jsmv_api::http::HttpApi, context);
    realm.register_api(jsmv_api::encoding::EncodingApi, context);
    realm.register_api(jsmv_api::ConsoleApi, context);
}
...
pub mod run {
    ...
    pub fn execute(
        ...
        let rt = &mut jsmv_core::Runtime::new(gas_limit)?;
        ...
        let result: JsValue = {
            ...
            runtime::with_host_runtime(hrt, || {
                jsmv_core::future::block_on(async move {
