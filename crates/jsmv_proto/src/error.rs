...
pub enum Error {
    CoreError { source: jsmv_core::Error },
    CryptoError { source: jsmv_crypto::Error },
    BalanceOverflow,
    InvalidNonce,
    InvalidAddress,
...
impl From<boa_engine::JsError> for Error {
    fn from(source: boa_engine::JsError) -> Self {
        Error::CoreError {
            source: jsmv_core::Error::JsError { source },
        }
    }
}

impl From<mavryk_smart_rollup::storage::path::PathError> for Error {
    fn from(source: mavryk_smart_rollup::storage::path::PathError) -> Self {
        Error::CoreError {
            source: jsmv_core::Error::PathError { source },
        }
    }
}
