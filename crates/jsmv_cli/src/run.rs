...
use jsmv_proto::operation::{Content, Operation, RunContract, SignedOperation};
use url::Url;
...
pub async fn exec(
    ...
    // Send message
    cfg.jsmv_client()?.post_operation(&signed_op).await?;

    let receipt = jsmv_client.wait_for_operation_receipt(&hash).await?;
